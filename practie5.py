...
class Obstacle(pygame.sprite.Sprite):
    def __init__(self,x,y,s):
        ...
        if s == 'car':
            ...
            self.character = pygame.image.load('grave.png')
        elif s == 'tree':
        ...
        self.rect = pygame.Rect(self.x,self.y,self.width,self.height)
        self.image = pygame.Surface((self.width,self.height),pygame.SRCALPHA)
        self.depthRect = pygame.Rect(self.x,(self.y+50),self.width,(self.height-50))
    ...
    def getrect(self):
        ...
        self.depthRect = pygame.Rect(self.x,(self.y+50),self.width,(self.height-50))
        Stop_Collide_List[self] = (self.depthRect)
...
def UpdateMap(Source,grup1,grup2):
    if Source.dirs == [0,0,1,0]:
        for sprit in RelativeMotionList:
            ...
            sprit.getrect()
            MainCh.getrect()
        if Source.depthRect.collidedict(grup2)!= None:
            print('coll')
            for sprit in RelativeMotionList:
                sprit.x -= Source.vx 
                MainCh.getrect()
    ...
    for tile in TileList:
        if (tile.x<-newwww.width):#(-1*newwww.width)):
            tile.x +=width+newwww.width
        if (tile.x>width):
            tile.x -=width+newwww.width
        if (tile.y<newwww.height):
            tile.y += height+newwww.height
        if (tile.y>height):
            tile.y -= height+newwww.height
...
def drawOverlay():
    ...
    if MainCh.gunlist[0] == 'pistol':
        screen.blit(pygame.image.load('pistolbanner.png'),(0,0))
    ...
    text3 = font.render(str(MainCh.lives), True, (255, 255, 0))
    screen.blit(text1,(50,12))
    screen.blit(text2,(390,12))
...
def redrawGame(screen):
    ...
    if MainCh.hurting:
        animateHurt(MainCh,AnimHurt)
    drawOverlay()
    pygame.draw.rect(screen,(0,0,0),MainCh.depthRect)
    pygame.draw.rect(screen,(0,0,0),Car.depthRect)
...
while playing:
    ...
    if keys[pygame.K_UP]:
        MainCh.dirs = [1,0,0,0]
        MainCh.updateChr()
        UpdateMap(MainCh,Slide_Collide_Group,Stop_Collide_List)
